...
const DASHBOARD_METRICS_CACHE_TTL_MS = Number.parseInt(
  process.env.DASHBOARD_METRICS_CACHE_TTL_MS || '',
  10
) || 5000;
...
class DashboardService {
  ...
  async getMetrics(user = null) {
    const cacheKey = user?.username ? `user:${user.username}` : 'system';
    const now = Date.now();
    ...
    const computePromise = (async () => {
      ...
      const getValue = (result, defaultVal, errorLabel) => {
        ...
        console.error(
          `[ERROR] DashboardService: ${errorLabel} failed:`,
      ...
      const correspondentCount = getValue(
        correspondentCountResult,
        0,
        'correspondentCount'
      );
      const paperlessHealthData = getValue(
        paperlessHealth,
        { healthy: false, documentCount: 0 },
        'paperlessHealth'
      );
      const dbHealthData = getValue(
        dbHealth,
        { healthy: false, documentCount: 0 },
        'dbHealth'
      );
      ...
      const processedDocumentCount = getValue(
        processedDocumentCountResult,
        dbHealthData.documentCount || 0,
        'processedDocumentCount'
      );
      const metrics = getValue(metricsResult, [], 'metrics');
      const processingTimeStats = getValue(
        processingTimeStatsResult,
        {},
        'processingTimeStats'
      );

      let tokenDistribution = getValue(
        tokenDistributionResult,
        [],
        'tokenDistribution'
      );
      ...
      const processingStatus = getValue(
        processingStatusResult,
        { isProcessing: false, processedToday: 0 },
        'processingStatus'
      );
      ...
      if (user && user.username) {
        try {
          recentActivity = await documentModel.getPaginatedHistory(
            5,
            0,
            user.username
          );
          ...
          console.error(
            '[ERROR] DashboardService: getPaginatedHistory failed:',
            err.message
          );
      ...
      // Token Stats
      const averagePromptTokens = metrics.length > 0
        ? Math.round(
          metrics.reduce((acc, cur) => acc + (cur.promptTokens || 0), 0)
            / metrics.length
        )
        : 0;
      const averageCompletionTokens = metrics.length > 0
        ? Math.round(
          metrics.reduce((acc, cur) => acc + (cur.completionTokens || 0), 0)
            / metrics.length
        )
        : 0;
      const averageTotalTokens = metrics.length > 0
        ? Math.round(
          metrics.reduce((acc, cur) => acc + (cur.totalTokens || 0), 0)
            / metrics.length
        )
        : 0;
      const tokensOverall = metrics.length > 0
        ? metrics.reduce((acc, cur) => acc + (cur.totalTokens || 0), 0)
        : 0;
      ...
      try {
        ...
        if (
          fullConfig &&
          (
            fullConfig.AI_PROVIDER ||
            fullConfig.OPENAI_API_KEY ||
            fullConfig.OLLAMA_API_URL
          )
        ) {
          aiHealth = 'online';
        }
    ...
    dashboardMetricsCache.set(cacheKey, {
      ...
      expiresAt: now + DASHBOARD_METRICS_CACHE_TTL_MS
    });
    ...
    return computePromise
      .then((value) => {
        dashboardMetricsCache.set(cacheKey, {
          hasValue: true,
          value,
          expiresAt: Date.now() + DASHBOARD_METRICS_CACHE_TTL_MS
        });
        return value;
      })
      .catch((error) => {

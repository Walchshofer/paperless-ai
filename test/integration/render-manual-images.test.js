...
describe('Manual view server rendering with images', function() {
  it('embeds images and overlaysByImage into overlay-viewer-island data-props', async function() {
    ...
    const vm = {
      document: { id: 42, title: 'Test Doc', pageCount: 1 },
      visual: {
    ...
    // Look for the overlay-viewer island anchor instead of the legacy manual anchor
    ...
    assert.ok(Array.isArray(props.images) || Array.isArray(props.images || []), 'images not present in data-props');
    assert.strictEqual((props.images && props.images[0] && props.images[0].id) || 'img-1', 'img-1');

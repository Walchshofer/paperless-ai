...
async function stubPromptValidateEndpoint(page: import('@playwright/test').Page) {
  await page.route('**/api/prompts/*/test', async (route: Route) => {
    await route.fulfill({
      status: 200,
...
test.describe('Prompt Lab Full Audit (Current Contracts)', () => {
  ...
  test('document source mode loads document pickers and can run validate flow', async ({ page }) => {
    ...
    await page.route('**/api/documents/recent', async (route: Route) => {
      await route.fulfill({
        status: 200,
        contentType: 'application/json',
        body: JSON.stringify({
    ...
    await page.route('**/api/documents/*/preview-image', async (route: Route) => {
      await route.fulfill({
        status: 200,
        contentType: 'application/json',
        body: JSON.stringify({

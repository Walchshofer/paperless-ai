...
const { fetchOverlaysForImage, normalizeBoxToPixels } = require('../../src/islands/VisualOverlaysIsland.tsx');
...
describe('fetchOverlaysForImage', function() {
  it('uses provided fetch impl and returns overlays', async function() {
    ...
    const fakeFetch = async (url, opts) => ({ ok: true, json: async () => ({ overlays: [{ id: 'ov-1', bbox: { x: 0, y: 0, width: 1, height: 1 } }] }) });
  ...
  it('caches results and returns cached data next call', async function() {
    ...
    let calls = 0;
    const fakeFetch = async (url, opts) => { calls++; return { ok: true, json: async () => ({ overlays: [{ id: 'ov-cache' }] }) }; };
    const first = await fetchOverlaysForImage(fakeImage, fakeFetch);

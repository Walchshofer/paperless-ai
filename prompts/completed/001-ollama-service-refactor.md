...
The complete refactored `ollamaService.js` is available at `.claude/ollamaService.REFACTORED.js`. Use this as the source for the new implementation. Key changes:

- **Inline utility functions**: All utility functions are now self-contained within the service
- **Dynamic timeout configuration**: Configurable via environment variable
- **Fixed token calculation**: Accurate estimation for Llama-based models
- **Enhanced error handling**: Comprehensive validation and graceful degradation
- **Robust response processing**: Handles edge cases and invalid responses
...
The complete refactored code is in `.claude/ollamaService.REFACTORED.js` - use this file as the template for replacing `services/ollamaService.js`.

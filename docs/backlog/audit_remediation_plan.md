...
|----|-----------|-------|-------|--------|
| **T-001** | `HistoryTabsIsland.tsx` | Fix `aria-*` invalid usages (e.g. `aria-controls` pointing to non-existent ID). Run axe tests. | Frontend | 🔴 Open |
...
| **T-004** | `SettingsSidebarIsland.tsx` | Fix accessibility visibility logic (`aria-hidden` vs `display:none`). | Frontend | 🔴 Open |

## 🔶 High / Medium (Stability & Correctness)

| ID | Component | Issue | Owner | Status |
|----|-----------|-------|-------|--------|
| **T-005** | CI Workflows | Fix env/secret context access warnings in `alpha9-fast-audit.yml` & `verification-fast.yml`. | Infra | 🟡 In Progress |

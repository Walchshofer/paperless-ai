...
- POST `/settings` route updates `data/runtime.env` (runtime-safe keys only)
...
- Loads runtime overrides from `data/runtime.env`; protected infra/secrets are sourced from `docker-compose.env`
...
### Decision 6: Runtime Env Scope (Updated Policy)

**Decision**: Only runtime-safe settings are written to `data/runtime.env`.
Protected infrastructure/secrets remain in `docker-compose.env`.

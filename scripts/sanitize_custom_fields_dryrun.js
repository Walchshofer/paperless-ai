// scripts/sanitize_custom_fields_dryrun.js
// Dry-run: lists documents that would be patched by sanitize_custom_fields.js
// Usage: node scripts/sanitize_custom_fields_dryrun.js
...
(async () => {
  try {
    ...
    console.log(`Documents that would be patched: ${affectedDocs}`);
    console.log(`Total custom field values that would be changed: ${affectedFields}`);
    if (affectedDocs === 0) {
      console.log('No patches required. All custom field values are compliant.');
    }
  } catch (err) {
    console.error('Error during dry-run sanitization:', err.message || err);
    process.exit(1);
  }
})();

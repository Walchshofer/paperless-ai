# PowerShell helper to purge pgvector artifacts
Write-Host "Backing up database to .\backups\paperless.dump"
...
Write-Host "Dropping extension and cleaning columns/indexes"
docker exec -i paperless_db psql -U elfman -d paperless -c "DROP EXTENSION IF EXISTS vector CASCADE;" 2>$null
# example index/column cleanup (adjust to your schema)
docker exec -i paperless_db psql -U elfman -d paperless -c "DROP INDEX IF EXISTS public.idx_visual_overlays_embedding_ivfflat;" 2>$null
docker exec -i paperless_db psql -U elfman -d paperless -c "ALTER TABLE public.visual_overlays DROP COLUMN IF EXISTS embedding;" 2>$null

Write-Host "Reindexing & refreshing collation"
...
Write-Host "Removing compiled artifacts and build deps (if present)"
...
docker exec paperless_db pg_isready -U ${env:POSTGRES_USER} -d ${env:POSTGRES_DB}

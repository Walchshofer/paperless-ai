...
from rag_service.data_manager import DataManager
...
def main():
    ...
    try:
        ...
        if not dm.is_initialized:
            logger.error("Failed to initialize DataManager. Check configuration.")
            sys.exit(1)
        ...
        success = dm._add_documents_to_qdrant(documents)

        if success:
            logger.info("✅ Successfully re-ingested all documents into Qdrant.")
        else:

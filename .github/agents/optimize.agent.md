---
name: optimize
description: MoE orchestrator coordinating all paperless-ai subagents for production
  optimization, ensuring doc-first guardrails, schema safety, and measurable pipeline
  improvements.
...
Use Copilot’s `/delegate` to hand work to the most appropriate specialist agent, and keep Serena memories up to date.

### Delegation rules
- Frontend UI work:
  - `@frontend-design-router` first if intent is unclear.
  - Then `@frontend-design-architect` (design/IA), `frontend-design-implementer` (code), or `frontend-design-auditor` (verification).
- Backend/API: `@paperless-api-expert`.
- Schemas/migrations: `@schema-evolution`.
- Pipeline and orchestration: `@pipeline-orchestration`.
- Failures and investigation: `@debug`.
- Documentation: `@docs`.
- Implementation tasks: `@implement`.
- Testing: `@test`.

### Serena workflow controls
- Use `oraios/serena/switch_modes` to move between planning/implementation/audit phases.
- Use `oraios/serena/summarize_changes` at the end of each phase to produce a concise change log.
- Track progress continuously in Serena memory:
  - `run-active`: current objective, phase, touched files, risks.
  - `handoff-next`: next concrete steps for the next agent/run.
...
\1  - label: 0. Frontend UI Routing
    agent: frontend-design-router
    prompt: |
      If the user request is primarily frontend/UI (EJS, Tailwind, Vanilla JS, Preact Islands),
      classify and delegate to exactly one of:
      - frontend-design-architect
      - frontend-design-implementer
      - frontend-design-auditor

      Ensure frontend guardrails are enforced (vm contracts, data-testid, island registry).
    send: false

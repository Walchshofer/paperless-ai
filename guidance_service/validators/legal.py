...
Handles:
- legal_classifier: dokumenttyp, komplexitaet,
  vermutete_jurisdiktion, vertrauen
- legal_extractor: vertragsparteien, daten, jurisdiktion_und_recht,
  vertrauen
- legal_validator: validation results with issues list
...
def validate_legal_extraction(
    ...
    - legal_classifier: dokumenttyp, komplexitaet,
      vermutete_jurisdiktion, vertrauen
    - legal_extractor: vertragsparteien, daten,
      jurisdiktion_und_recht, vertrauen
    ...
    try:
        ...
        if "vertragsparteien" in data:
            ...
            errors.extend(
                _validate_extractor_output(data, warnings)
            )
        elif (
            "dokumenttyp" in data
            and "komplexitaet" in data
        ):
            ...
            if not isinstance(data.get("valid"), bool):
                errors.append(
                    "Legal validator returned invalid valid flag"
                )
            elif not data.get("valid"):
                errors.append(
                    "Legal validator marked output invalid"
                )
...
def _validate_extractor_output(
    ...
    if dates:
        ...
        if abschluss and not _is_valid_date(abschluss):
            warnings.append(
                f"Invalid contract date format: {abschluss}"
            )

        gueltig_ab = dates.get("gueltig_ab")
        if gueltig_ab and not _is_valid_date(gueltig_ab):
            warnings.append(
                f"Invalid effective date format: {gueltig_ab}"
            )
    ...
    if jurisdiction:
        applicable_law = (
            jurisdiction.get("anwendbares_recht", "")
        )
        valid_laws = [
            "Österreich (ABGB)",
            "Deutschland (BGB)",
        ...
        if applicable_law and applicable_law not in valid_laws:
            ...
            warnings.append(
                f"Non-standard applicable law: {applicable_law}"
            )
...
def _is_valid_date(date_str: str) -> bool:
    ...
    Returns:
        True if valid date format or null/empty, False otherwise
    """
    if not date_str or date_str == "null":
        return True  # Null/empty dates acceptable
...
def _validate_tag_fields(
    ...
    Args:
        ...
        domain: Expected domain ("legal", "medical", etc.)
    """
